...
class TestMockDataExtract:
    def test_extract_traffic_cards(self, mock_browser):
        ...
        assert cards['edge'] == {'value': 12.34, 'unit': 'TB'}
        assert cards['origin'] == {'value': 4.56, 'unit': 'TB'}
        assert cards['midgress'] == {'value': 7.89, 'unit': 'GB'}
        assert cards['offload'] == {'value': 56.78, 'unit': '%'}

    def test_extract_geography_table(self, mock_browser):
        ab_eval("window.location.hash = '#/predefined/traffic-by-geography'")
        time.sleep(1)
        geo = extract_geography_table(['ID', 'TW', 'SG'])
        assert geo['ID'] == bytes_to_tb(9_870_000_000_000)
        assert geo['TW'] == bytes_to_tb(21_000_000_000)
        assert geo['SG'] == bytes_to_tb(3_400_000_000)
...
class TestMockCpCodes:
    ...
    def test_search_and_select(self, mock_browser):
        ...
        run_ab('fill', "input[placeholder='CP codes']", '100001')
        time.sleep(1)
        run_ab('click', '#cpcodes-filter-editor >> text=(100001)')
        time.sleep(0.5)
        state = json.loads(ab_eval('window.__mockState'))
        assert '100001' in state['selectedCpCodes']
